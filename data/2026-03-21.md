...
TL;DR: 机器学习分子动力学（MLMD）为铁电材料极化动力学研究提供了量子精度与大尺度模拟的平衡方案，可模拟极化翻转、畴结构、拓扑极化和曲率驱动现象，但面临长程静电、自旋耦合等挑战。


<details>
  <summary>Details</summary>
Motivation: 铁电材料的性能与稳定性取决于极化动力学和畴动力学，需要微观理解和原子级控制。实验表征技术有限，第一性原理计算受限于尺度，需要能平衡精度与尺度的模拟方法。

Method: 采用机器学习分子动力学（MLMD），将量子力学精度编码到力场中，实现接近第一性原理精度的大尺度动态模拟。该方法可应用于极化翻转、畴成核与迁移、拓扑极化纹理和曲率驱动铁电现象的研究。

Result: MLMD在铁电材料模拟中显示出优势，能够捕捉复杂动态过程特征。通过系统综述该领域进展，识别了限制预测能力的方法学挑战，包括长程静电、多铁材料中的晶格-自旋耦合自由度、以及数据高效预训练大型原子模型。

Conclusion: MLMD为铁电和多铁材料设计提供了有前景的预测框架。未来需要在长程感知力场、自旋依赖机器学习模型和大规模预训练方面取得进展，以推动MLMD成为真正的预测性工具。
...
TL;DR: 在ZnP2中发现超长光生载流子寿命，填补了直接带隙无机半导体与卤化物钙钛矿之间的载流子寿命差距


<details>
  <summary>Details</summary>
Motivation: 传统无机半导体需要严格控制制备条件才能获得长载流子寿命，而卤化物钙钛矿在相对不受控环境中也能实现长寿命。研究旨在发现具有类似优异性能的新型无机材料

Method: 通过计算筛选识别ZnP2作为长寿命半导体，实验合成ZnP2晶体并测量其光致发光和载流子寿命，分析其非常规聚磷化物键合结构

Result: ZnP2晶体在1.49 eV处表现出明亮的带间光致发光，载流子寿命接近1微秒，其聚磷化物键合抑制了深能级本征缺陷形成，具有缺陷抗性

Conclusion: ZnP2填补了无机半导体与钙钛矿之间的载流子寿命差距，具有优异的环境稳定性，是太阳能吸收体和发光器件的理想材料。非常规化学键合的无机材料空间具有巨大潜力
...
TL;DR: 该研究建立了玻璃中扩散的定量框架，发现相关运动而非局部重排势垒主导激活能，解释了低势垒重排如何产生高宏观激活能


<details>
  <summary>Details</summary>
Motivation: 晶体中的扩散通过缺陷介导的原子跳跃得到定量理解，但玻璃中缺乏类似的定量框架。玻璃中局部重排涉及低势垒，却产生大的扩散激活能，这一矛盾令人困惑

Method: 使用金属玻璃的分子动力学模拟，将扩散分解为随机行走和相关贡献两部分进行分析。研究扩展到SiO2和单组分Lennard-Jones玻璃以验证普适性

Result: 发现来回相关运动主导激活能，而非局部重排势垒。这种相关性源于无序能量景观中正向和反向势垒的不对称性。该机制在多种玻璃体系中普遍存在，单组分Lennard-Jones玻璃证明相关性源于结构无序而非化学复杂性。框架还能解释表面扩散加速现象

Conclusion: 建立了原子尺度动力学与宏观输运之间的直接定量联系，为无序材料中的动力学提供了预测基础，解决了玻璃中扩散激活能来源的长期困惑
...
TL;DR: 该研究通过原子尺度理论计算和实验相结合，揭示了Sb2Te相变材料的非常规光学特性，提出"越短越好"策略，实现了单波导单元超过7比特的光学编程精度，创造了全光学相变存储器件的新纪录。


<details>
  <summary>Details</summary>
Motivation: 光子神经形态计算需要每个相变单元尽可能多的光学可编程级别，同时保持相对较低的光学损耗。目前需要深入理解相变材料的光学特性以指导器件设计。

Method: 采用原子尺度的理论计算与实验相结合的方法，研究典型生长驱动相变材料Sb2Te的亚稳态晶态光学特性。基于这些发现，提出"越短越好"的设计策略用于Sb2Te基全光学波导器件。

Result: 实现了单波导单元超过7比特的光学编程精度，这是全光学相变存储器件的最高纪录。同时改善了编程窗口和光学损耗性能。

Conclusion: 这项工作展示了"从原子到器件"方案的典型范例，证明了深入原子尺度理解在指导相变光子器件设计以提高性能方面的预测能力。
...
TL;DR: 该研究通过实验和理论结合的方法，以SrTiO3为模型系统，揭示了非阿伦尼乌斯晶粒生长的热激活本质及其温度依赖性特征。


<details>
  <summary>Details</summary>
Motivation: 非阿伦尼乌斯晶粒生长在多种多晶材料中被观察到，但其基本机制（特别是该过程是热激活还是反热激活）仍存在争议，需要系统研究澄清。

Method: 以SrTiO3为模型系统，采用实验与理论相结合的方法，利用新开发的晶粒生长模型系统研究非阿伦尼乌斯晶粒生长行为。

Result: 非阿伦尼乌斯晶粒生长是热激活过程，没有确定的特征温度，主要由温度依赖因素与温度无关参数（如晶粒尺寸及其分布）的相互作用控制；在异常晶粒生长过程中，非阿伦尼乌斯行为主要发生在较低温度，随温度升高逐渐转变为阿伦尼乌斯型行为。

Conclusion: 研究明确了非阿伦尼乌斯晶粒生长的热激活本质，揭示了其温度依赖性转变特征，为理解多晶材料中晶粒生长机制提供了重要理论依据。
...
<details>
  <summary>Details</summary>
Motivation: 研究Cr:YAG透明陶瓷在红外激光激发下产生的白光发射现象，探索其产生机制和条件，为理解激光与材料相互作用提供新见解

Method: 使用红外激光激发Cr:YAG透明陶瓷，观察白光发射现象，测量临界功率阈值，分析表面与体相差异，在真空条件下进行实验验证

Result: 发现陶瓷在超过临界功率阈值时产生明亮的白光发射，该现象仅在样品表面两侧观察到而非体相，且必须在真空条件下才能出现

Conclusion: 激光诱导白光发射现象可用Cr3+/Cr4+离子对间的价间电荷转移机制来解释，该机制在特定条件下（表面、真空、超临界功率）被激活
...
TL;DR: 研究非辐射弛豫过程对Cr:YAG纳米粉末激光诱导白光发射(LIWE)性质的影响，发现铬浓度增加会提高N参数(参与LIWE过程的光子数)，这与非辐射复合过程概率增加相关，并提出了多光子电离模型解释LIWE现象。


<details>
  <summary>Details</summary>
Motivation: 激光诱导白光发射(LIWE)虽已发展到工业应用阶段，但对现象本质的理解不足限制了其潜力。本文旨在研究非辐射弛豫过程对Cr:YAG纳米粉末LIWE性质的影响，以深入理解该现象。

Method: 采用Pechini方法合成Cr:YAG纳米粉末浓度系列，研究其微观结构、光学性质和LIWE特性。分析铬浓度对N参数(参与LIWE过程的光子数)的影响，并提出多光子电离模型来解释LIWE现象。

Result: 铬浓度增加导致N参数升高，这与非辐射复合过程概率的增加相关。实验表明非辐射弛豫过程对LIWE性质有显著影响，支持了多光子电离模型对LIWE现象的解释。

Conclusion: 非辐射弛豫过程在LIWE现象中起关键作用，铬浓度通过影响非辐射复合概率来调控LIWE性质。提出的多光子电离模型为理解LIWE现象提供了理论框架，有助于推动该技术的进一步发展和应用。
...
TL;DR: 研究Yb含量对Cr,Yb:YAG纳米晶体激光诱导白光发射(LIWE)特性的影响，揭示Cr3+与Yb3+离子间的能量转移过程，并用多光子电离理论解释实验结果。


<details>
  <summary>Details</summary>
Motivation: 尽管激光诱导白光发射(LIWE)现象已被广泛研究数十年，但其机制仍不明确。本研究旨在通过调控Yb含量来深入理解Cr,Yb:YAG纳米晶体中LIWE的特性与机制。

Method: 制备不同Yb含量的Cr,Yb:YAG纳米晶体样品，表征其微观结构和光学性质，分析Yb3+离子浓度对光谱特性的影响，研究Cr3+与Yb3+离子间的能量转移过程，并应用多光子电离理论解释实验现象。

Result: 揭示了Yb含量对Cr,Yb:YAG纳米晶体LIWE特性的显著影响，阐明了Cr3+与Yb3+离子间的能量转移机制，多光子电离理论成功解释了观察到的实验现象。

Conclusion: Yb含量是调控Cr,Yb:YAG纳米晶体LIWE性能的关键参数，Cr3+与Yb3+离子间的能量转移过程在LIWE机制中起重要作用，多光子电离理论为理解该现象提供了理论框架。
...
TL;DR: 该研究首次使用软X射线角分辨光电子能谱直接探测了SiGe势垒中埋入的应变Ge量子阱的电子结构，揭示了应变和量子限制如何决定Ge量子阱的价带结构。


<details>
  <summary>Details</summary>
Motivation: Ge/SiGe异质结构是空穴自旋量子技术和高迁移率电子学的有前景平台，但迄今为止，埋入应变Ge量子阱的动量分辨价带结构只能间接推断，缺乏直接实验观测。

Method: 采用软X射线角分辨光电子能谱直接探测SiGe势垒中埋入的应变Ge量子阱的电子结构，并结合第一性原理计算进行分析。

Result: 成功解析了应变分裂和尺寸量子化的价带子带，确定了它们的重空穴、轻空穴和自旋轨道分裂成分，测量了Ge/SiGe异质结的价带偏移，发现准确描述需要明确包含SiGe势垒施加的限制势。
...
<details>
  <summary>Details</summary>
Motivation: 设计能够有效捕捉非谐性且计算成本低的材料描述符是材料设计中的关键挑战，这对于调控晶格热导率至关重要

Method: 提出"声子带中心"描述符，该描述符封装了声子散射物理的关键因素，与Grüneisen参数呈简单反比关系，并与晶格热导率强相关。首先在黄铜矿类材料中建立该指标，随后使用实验热导率数据在多种材料类别中进行验证

Result: PBC描述符能够有效区分材料，揭示与晶格热导率的强相关性，简化了具有理想热导率特性的候选材料的识别过程

Conclusion: 声子带中心是一个简单有效的描述符，能够量化非谐效应并预测晶格热导率，为材料热导率设计提供了实用工具
...
TL;DR: 通过实验测量和理论计算研究了金红石相GeO₂的热导率各向异性及其温度依赖性，揭示了其微观机制


<details>
  <summary>Details</summary>
Motivation: 金红石相GeO₂作为超宽带隙半导体在功率电子器件中具有应用潜力，但其热传输的各向异性及其微观机制尚未通过实验解决，而高效散热对器件可靠性至关重要

Method: 结合温度依赖的时域热反射测量和第一性原理声子输运计算，量化了单晶金红石GeO₂在80-350K温度范围内沿[001]和[110]方向的热导率

Result: 295K时，沿[001]方向热导率为47.5 W m⁻¹ K⁻¹，沿[110]方向为32.5 W m⁻¹ K⁻¹，各向异性比为1.46；热导率呈现T⁻¹.⁴的温度依赖性，表明存在超出三声子散射的额外散射机制；模式分辨分析显示室温各向异性源于[001]方向更大的声子群速度和方向依赖的声子寿命

Conclusion: 研究建立了金红石GeO₂体相和界面热传输的微观基础，将该材料定位为超宽带隙电子学中具有热稳定性的有前景平台
...
TL;DR: 利用扫描SQUID显微镜研究V掺杂(Bi,Sb)₂Te₃量子反常霍尔效应中的磁畴结构，发现磁畴与晶粒尺寸相似，磁反转通过畴壁扩展而非随机成核，揭示了局域磁相互作用与长程铁磁耦合的共存。


<details>
  <summary>Details</summary>
Motivation: 量子反常霍尔效应在磁性掺杂拓扑绝缘体中被观察到，但其磁有序性质存在争议——实验证据同时支持长程和短程有序。需要明确这些系统中磁性的精确性质，特别是V掺杂(Bi,Sb)₂Te₃在精确量子化条件下的磁畴结构。

Method: 使用扫描超导量子干涉器件显微镜研究V掺杂(Bi,Sb)₂Te₃样品，通过成像杂散磁场随外加磁场的变化，绘制磁畴形成和演化过程。重构测量到的杂散磁场对应的磁化构型，分析磁畴与晶粒尺寸关系以及磁反转机制。

Result: 1. 磁畴尺寸与晶粒尺寸相似；2. 磁反转通过畴壁扩展发生，这是铁磁体的典型特征，而非随机成核；3. 揭示了局域磁相互作用（晶粒内）与长程铁磁耦合（晶粒间）的共存；4. V掺杂(Bi,Sb)₂Te₃的行为与先前报道的Cr掺杂(Bi,Sb)₂Te₃明显不同。

Conclusion: V掺杂(Bi,Sb)₂Te₃量子反常霍尔系统表现出独特的磁性行为：磁畴与晶粒结构紧密相关，磁反转机制表明存在长程铁磁耦合，同时晶粒内保持局域磁相互作用。这种磁性构型与Cr掺杂系统存在显著差异，为理解拓扑绝缘体中磁性掺杂的机制提供了新见解。
...
<details>
  <summary>Details</summary>
Motivation: 将不同维度的半导体异质结构组合可以调控材料物理性质，但二维钙钛矿微晶与钙钛矿纳米晶超晶格由于溶解性不匹配和转移困难，难以构建异质结构。

Method: 开发了一种简单有效的方法，在PEA2PbBr4二维层状钙钛矿微晶表面确定性生长CsPbBr3纳米晶超晶格，可形成核-冠（仅侧面）或核-壳（全方向）异质结构。

Result: 成功构建了异质结构，展示了其作为高效光捕获系统的功能：能量可从二维微晶域转移到超晶格；通过调节激发通量可在线性和非线性载流子复合机制间切换；利用纳米晶发射寿命随温度降低而缩短的特性，确保能量转移发生在双激子和单激子衰减之后，有效延长超晶格的辐射复合。

Conclusion: 该方法简单、有效且通用，有望扩展到多种其他材料体系。构建的异质结构在光捕获和能量转移方面表现出优异性能，为多功能光电应用提供了新平台。
...
TL;DR: 该论文提出了一种在具有非共线磁亚晶格的铁电体中识别时间反演对称的p波和f波自旋极化绝缘电子态的新策略，为低耗散电子学提供了新途径。


<details>
  <summary>Details</summary>
Motivation: 铁电序与磁序之间的耦合为低耗散电子学提供了有前景的途径，但由于绝缘能带结构与铁磁性之间的兼容性差，这种耦合非常罕见。论文旨在探索一种不同的策略来解决这一问题。

Method: 结合自旋和磁群论对极性对称破缺的起源进行系统分类，区分晶体学、交换作用或自旋轨道驱动的机制。通过第一性原理计算，在已知的多铁材料GdMn₂O₅中演示了原始的时间反演对称p波自旋极化电子结构。

Result: 识别了超过50种候选材料，并在GdMn₂O₅中证明了其p波自旋极化电子结构可以通过电学方式切换，为该类材料在自旋电子学和多铁性功能方面开辟了新的路径。

Conclusion: 通过识别非共线磁亚晶格铁电体中的时间反演对称自旋极化绝缘态，为铁电-磁耦合提供了新机制，为低耗散电子器件设计提供了新的材料平台和物理基础。
...
TL;DR: 该研究通过多模式扫描探针显微镜揭示了CuInP2S6中光激发与铁电极化、离子迁移的协同耦合机制，建立了"光-铁电-离子"耦合模型，为基于层状铁电材料的可光寻址存储器、光电开关和神经形态器件设计提供了纳米尺度框架。


<details>
  <summary>Details</summary>
Motivation: 理解光激发在范德华铁电材料中如何与极化和界面静电学耦合，对于开发光可编程纳米电子和光电器件至关重要。CuInP2S6作为范德华铁电半导体，其光-铁电-离子耦合机制尚不明确。

Method: 采用相关多模式扫描探针显微镜技术：开尔文探针力显微镜、压电力显微镜和导电原子力显微镜，在带隙以上光照条件下系统研究CuInP2S6的光响应特性。

Result: 光照诱导表面功函数增强、持续表面光电压、矫顽场降低和正印移偏移。这些效应源于光生载流子重分布和Cu+离子缓慢迁移的协同作用，重塑了界面耗尽层宽度和内建电场。光照辅助势垒降低进一步增强了载流子注入，产生扫描速率依赖的铁电-离子输运迟滞。

Conclusion: 研究确立了"光-铁电-离子"耦合作为CuInP2S6中光控能带调制和极化稳定性的主导机制，为基于层状铁电-离子材料的光可寻址铁电存储器、光电开关和神经形态器件设计提供了纳米尺度框架。
...
TL;DR: 引入自旋-电荷群（SCG）作为描述费米子系统最一般对称性的新框架，涵盖自旋、电荷、时空自由度及其耦合，用于探索新物相
...
Method: 引入自旋-电荷群（SCG），包含自旋和电荷操作作为"内部"对称性，时空操作作为"外部"对称性，它们之间的耦合以及投影扭曲；研究SCG在具体物理系统中的应用

Result: SCG能够产生额外的能带简并、陈数和交叉自旋-电荷响应（如自旋超流）；为分类和探索新物相提供途径，即使包含强相互作用

Conclusion: 自旋-电荷群提供了一种描述费米子系统最一般对称性的框架，能够处理自旋、电荷和时空自由度之间的复杂耦合，为探索新物相开辟了道路
...
TL;DR: 该研究揭示了对称性约束的自旋-晶格耦合导致模式选择性自旋-声子相互作用，从而控制热输运，为自旋轨道耦合莫特绝缘体中的场致输运异常提供了微观解释。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解释各种受挫磁体中磁热输运的场依赖异常现象，这些现象偏离了传统的磁振子或声子输运机制，特别是在自旋轨道耦合莫特绝缘体中观察到的场致输运异常。

Method: 在强自旋轨道耦合极限下推导有效自旋-声子哈密顿量，其中不同极化的声子选择性地耦合到不同的自旋算符；采用朗道尔输运框架结合自旋链与声子浴耦合的精确对角化方法。

Result: 压缩和剪切声子模式在不同磁场区域对自旋热流有不同贡献，该机制在热流的场依赖关系中产生特征性的峰-谷-峰结构，为实验观测到的场致输运异常提供了微观解释。

Conclusion: 对称性约束的自旋-晶格耦合通过模式选择性的自旋-声子相互作用控制热输运，这是理解自旋轨道耦合莫特绝缘体中磁热输运异常的关键机制。
...
<details>
  <summary>Details</summary>
Motivation: 变分蒙特卡洛方法在实际应用中面临统计病理问题：连续空间中的重尾估计器方差可能发散，离散希尔伯特空间中的采样分布可能遗漏支撑集部分，导致优化轨迹不可靠或变分动力学错误

Method: 引入模糊采样方法，这是一种后处理技术，无需修改底层采样器，通过处理波函数节点问题来获得良好行为的统计估计器

Result: 在标准采样方法已知失效的几个代表性示例中证明了模糊采样的有效性，并将其应用于大规模自旋动力学问题

Conclusion: 建立了一个广泛适用的鲁棒VMC和t-VMC计算框架，解决了波函数节点导致的统计病理问题
...
TL;DR: 通过第一性原理计算，研究了掺杂诱导的小极化子如何影响Ba₂CaOsO₆中5d²双钙钛矿的低温多极序，发现极化子反转了主导的八极交换作用，解释了Na掺杂对铁八极序的抑制效应。


<details>
  <summary>Details</summary>
Motivation: 研究掺杂诱导的小极化子对5d²双钙钛矿Ba₂CaOsO₆低温多极序的影响，澄清先前实验中Na掺杂导致铁八极序被抑制的模糊观察结果。

Method: 采用第一性原理计算5d¹局域空穴极化子与5d²磁性离子之间的位点间交换相互作用，构建相应的有效哈密顿量并求解，分析交换作用反转对多极序的影响。

Result: 计算表明掺杂诱导的小极化子将主导的八极交换作用从铁磁性反转为反铁磁性，这解释了Na掺杂对铁八极序的渐进抑制和有序温度的降低。

Conclusion: 电荷掺杂形成的小极化子为调控自旋轨道纠缠材料中的位点间交换相互作用提供了可行途径，能够实现新型量子序的出现，澄清了先前实验观察中的模糊性。
...
Method: 扩展了半填充条件下的归一化流方法到有限化学势，通过引入退火方案实现遍历性采样。该方法在自旋基中工作，相比电荷基中的混合蒙特卡洛方法具有优势。

Result: 与电荷基中最先进的混合蒙特卡洛方法相比，该方法准确再现了精确对角化结果，同时将统计不确定性降低了一个数量级。

Conclusion: 该方法为掺杂关联系统的模拟开辟了新途径，解决了有限化学势下Hubbard模型的符号和遍历性问题，显著提高了模拟精度和效率。
...
Method: 通过将哈密顿量映射到Majorana表示，从与哈密顿量对易的算符的李代数获得连续对称性的生成元；利用半单李代数理论识别该李代数的结构；基于表示理论开发系统方法，通过分解Majorana空间上对称代数诱导的外幂表示并结合离散晶格对称性，穷举枚举候选序参量。

Result: 建立了一个系统框架，能够分析连续对称性并识别候选序参量，根据它们破坏的对称性对这些序参量进行分类。
...
TL;DR: 研究强关联d轨道电子系统中的间接激子凝聚，发现在位Hubbard U会显著抑制凝聚温度Tc，特别是在高电子-空穴对密度下，这与无U情况相反。三轨道模型中多配对通道竞争进一步抑制Tc，并识别出轨道选择性配对态。


<details>
  <summary>Details</summary>
Motivation: 理解过渡金属硫族化合物(TMC)中激子绝缘体基态的形成机制，特别是解释这些材料中强激子结合能与相对较低转变温度之间的巨大差异。这些材料的低能物理由强在位库仑相互作用的d轨道电子主导。

Method: 采用动力学平均场理论(DMFT)结合行列式量子蒙特卡洛方法(DQMC)，研究三角晶格上二维半金属中的空间间接激子凝聚。构建包含强在位相互作用(U)和轨道间相互作用(V)的双轨道和三轨道Hubbard模型。

Result: 在位Hubbard U会强烈抑制凝聚温度Tc，这种效应在高电子-空穴对密度下尤为显著，与无U情况下Tc随对密度增加而增长形成鲜明对比。在三轨道模型中，多个电子-空穴配对通道之间的竞争进一步抑制激子凝聚的Tc。识别出轨道选择性电子-空穴配对态。

Conclusion: 研究结果为理解TMC材料中间接激子的强结合能与相对较低转变温度之间的巨大差异提供了重要解释。这些发现为理解和设计具有强关联d壳层电子的材料中的激子凝聚提供了重要见解。
...
TL;DR: 在Kagome双层材料CsCr6Sb6中首次观测到平带共振现象，平带与色散带在低温下出现显著的谱权重增强和杂化，与短程反铁磁关联同时出现，不同于传统Kondo晶格行为。


<details>
  <summary>Details</summary>
Motivation: Kagome晶格同时具有平带（局域态）和色散带（巡游态），为研究局域与巡游电子相互作用提供了理想平台，但平带共振的直接光谱证据一直缺失。本研究旨在寻找并证实Kagome材料中的平带共振现象。

Method: 使用角分辨光电子能谱、输运测量，结合密度泛函理论和动力学平均场理论，研究准二维Kagome双层材料CsCr6Sb6的电子结构和物性。

Result: 在费米能附近发现了共存的平带双重态和色散带。降温过程中，平带和色散带表现出显著的谱权重增强和杂化，直接证实了平带共振。这一现象与短程反铁磁关联的起始同时出现，与传统Kondo晶格行为形成鲜明对比。

Conclusion: 研究不仅首次在Kagome材料中观测到长期寻找的平带共振现象，还揭示了其与磁性的非常规关联，为理解局域与巡游电子相互作用在关联和拓扑量子态中的作用提供了重要见解。
...
TL;DR: 通过激光光电子显微镜研究Kagome金属CsCr₃Sb₅顺磁态的费米面，发现布里渊区中心的圆形和六角形费米面，并在边界探测到小费米口袋，揭示了轨道依赖的关联效应。
...
Result: 发现布里渊区中心存在一个圆形费米面和两个六角形费米面，在边界探测到小费米口袋。d_{xz}轨道的费米面尺寸显著改变，表明存在轨道依赖的关联效应。

Conclusion: 研究揭示了CsCr₃Sb₅中轨道依赖的电子关联效应，为理解该材料中反铁磁/电荷密度波序与潜在非常规超导性的相互作用提供了关键电子结构信息。
...
TL;DR: 该理论研究了一维Mott绝缘体中光掺杂载流子的光电子能谱特征，发现其与半导体中的光电子能谱存在本质差异，揭示了强关联系统中分数化元激发对光谱响应的根本性改变。


<details>
  <summary>Details</summary>
Motivation: 研究强关联一维Mott绝缘体中光掺杂载流子的光电子能谱特征，揭示其与常规半导体光电子能谱的本质区别，探索通过光电子能谱获取强关联系统中磁性和载流子结合性质信息的可能性。

Method: 结合精确对角化方法和slave-particle方法，理论分析一维Mott绝缘体中光掺杂载流子的光电子能谱特征，特别关注自旋子、空穴子和双占据子等分数化元激发对光谱响应的贡献。

Result: 发现一维Mott绝缘体的光电子能谱特征与半导体存在本质差异：当光掺杂载流子未结合时，光谱直接反映自旋子的色散关系；当双占据子和空穴子形成激子束缚态时，Mott带隙内会出现下Hubbard带的复制结构，其分布对双占据子-空穴子结合程度敏感。

Conclusion: 强关联电子系统中光诱导载流子和激子的光电子能谱可以提供关于磁性质和载流子结合性质的重要信息，为研究强关联系统中的元激发和束缚态提供了新的光谱学手段。
...
TL;DR: 该研究报道了受挫磁体Co₃ZnNb₂O₉的合成与热力学性质，发现其在14K发生长程磁有序，具有强反铁磁相互作用和未淬灭轨道角动量，展现出场诱导的自旋翻转型亚铁磁转变和磁热效应。
...
Method: 合成了Co₃ZnNb₂O₉样品，通过温度依赖的磁化率测量分析其磁有序行为，利用居里-外斯拟合确定相互作用强度和有效磁矩，通过比热测量确认长程磁有序，研究场诱导的亚铁磁转变和磁热效应。

Result: Co₃ZnNb₂O₉在14K出现长程磁有序，居里-外斯温度为-70K表明强反铁磁相互作用，有效磁矩5.54μB显示未淬灭轨道角动量。在有序温度以下观察到1.2T临界场的自旋翻转型亚铁磁转变。比热在14K显示λ型异常。磁热效应显示2.81J/kg·K的熵变。

Conclusion: 受挫蜂窝磁体Co₃ZnNb₂O₉展现出由竞争相互作用、各向异性和净磁化强度减小导致的相对较小等温熵变，表明这类材料是场诱导奇异相和磁热响应的有前景候选体系。
...
Method: 开发了包含四种计算方案的框架，将熵表示为参数空间中的路径积分，被积函数仅涉及总能、费米子密度和双占据等基本可观测量；推导了连接熵与其他量的麦克斯韦关系；使用辅助场量子蒙特卡罗方法计算二维和三维掺杂哈伯德模型的熵。

Result: 不同方案间结果高度一致，定量验证了麦克斯韦关系；二维结果与图解蒙特卡罗计算在物理相关参数范围内表现出优异的定量一致性。
...
<details>
  <summary>Details</summary>
Motivation: 研究d波超导体中的逆法拉第效应，理解光诱导磁化现象在非常规超导体中的微观机制，特别是分支布居不平衡对非线性直流响应的影响。

Method: 采用扩展的Keldysh-Nambu准经典形式，计算外部单色辐射诱导的直流电流密度分量，理论框架考虑了非平衡态和非线性响应。

Result: 理论明确展示了分支布居不平衡如何产生非零的非线性和非局域直流响应，计算了诱导电流的大小并获得了诱导静态磁化的估计值。

Conclusion: 该工作为d波超导体中的逆法拉第效应提供了微观理论框架，讨论了实验意义并提出了未来工作的扩展方向。
...
TL;DR: 开发了基于第一性原理的非平衡超导响应模型，利用实频轴上的Migdal-Eliashberg方程定量描述光泵浦实验中超导薄膜的光学响应，预测了更多材料中可实现光诱导超导性


<details>
  <summary>Details</summary>
Motivation: 实验发现超导材料在远离平衡态时具有独特性质，但缺乏定量描述实验观测的第一性原理理论框架

Method: 开发了基于第一性原理的非平衡模型，在实频轴上直接求解Migdal-Eliashberg方程，采用新的数值技术处理电子-声子介导的超导性

Result: 定量再现了Pb和LaH10超导薄膜在泵浦探测实验中的光学响应，预测K3C60在超快中红外脉冲照射下产生光诱导超导态，并预测CaC6具有类似的光诱导超导能隙

Conclusion: 光诱导超导性在更多材料中可实现，其机制与微波照射下超导增强类似，为理解非平衡超导现象提供了定量理论框架
...
TL;DR: 该论文提出了一种在实频轴上直接求解有限温度Migdal-Eliashberg方程的方法，避免了传统虚轴计算所需的解析延拓，可直接获得谱函数和输运性质。


<details>
  <summary>Details</summary>
Motivation: 实验相关的超导特征需要实频量（如谱函数、光学响应、输运性质），但传统的Migdal-Eliashberg计算通常在虚轴上进行，然后通过解析延拓获得实频结果。这一过程数值上不稳定，可能掩盖物理相关的谱特征。现有实轴计算常采用恒定态密度近似，忽略了粒子-空穴不对称性。

Method: 提出了一种在实频轴上直接求解有限温度Migdal-Eliashberg方程的实用方法。该方法：1) 保留全带宽电子结构效应；2) 通过能量依赖的电子态密度考虑粒子-空穴不对称性，避免了恒定态密度近似；3) 包含静态屏蔽库仑贡献；4) 引入高效数值技术，计算成本随实频网格线性缩放，使高分辨率全带宽实轴计算可行；5) 可直接获得相互作用格林函数和可观测量，无需解析延拓。

Result: 将该方法应用于H₃S体系，其中费米能级附近的van-Hove奇点产生强粒子-空穴不对称性。全带宽解得到的谱与恒定态密度近似有明显差异，超导能隙和线形与实验更吻合，突显了能带结构细节的重要性。该方法为Eliashberg理论中的时间依赖非平衡模拟开辟了道路。

Conclusion: 开发了一种高效、实用的实频轴Migdal-Eliashberg求解方法，可直接获得超导体系的谱函数和输运性质，避免了传统虚轴计算中解析延拓的不稳定性。该方法特别适用于具有强粒子-空穴不对称性的体系，为研究复杂超导材料提供了更准确的计算工具。
...
TL;DR: 通过Nb蒸气沉积在加热的青铜片上形成扩散连接，同时生成Nb₃Sn薄膜跨越接缝，实现超导电流传输


<details>
  <summary>Details</summary>
Motivation: 开发新的Nb₃Sn材料连接方法，用于磁体导体和射频腔应用

Method: 将多对青铜片沿接缝对齐，在~715°C加热时通过溅射沉积暴露于Nb蒸气，同时进行扩散连接和Nb₃Sn转化；还尝试了预涂Nb的改进方法

Result: 热青铜方法成功生成跨越接缝的均匀Nb₃Sn薄膜，超导电流能在9K下自由流过接缝；预涂Nb方法效果较差

Conclusion: 热青铜方法可实现高质量的Nb₃Sn连接，为磁体导体和射频腔应用提供了新的连接技术途径
...
TL;DR: 研究二维颗粒铝薄膜中从均匀行为到非均匀（渗流）行为的转变，通过测量电阻和磁导率，获得异常温度依赖的扩散常数和渗流临界指数。


<details>
  <summary>Details</summary>
Motivation: 研究二维颗粒铝薄膜中电子输运行为从均匀到非均匀（渗流）的转变机制，理解渗流理论在超导薄膜系统中的适用性。

Method: 测量二维颗粒铝薄膜的电阻温度依赖性和磁阻，利用扩散常数作为拟合参数分析磁导率数据，从扩散常数温度依赖性提取渗流临界指数θ。

Result: 发现扩散常数异常温度依赖性，获得渗流临界指数θ；θ值在电阻约1.5kΩ处急剧变化，表明均匀-非均匀转变；渗流薄膜中α_T因子与电阻成反比关系，符合渗流标度律模型。

Conclusion: 二维颗粒铝薄膜中观察到从均匀到渗流行为的转变，电阻阈值约1.5kΩ；渗流薄膜的电导率温度依赖性符合渗流理论标度律，验证了渗流模型在超导薄膜系统中的适用性。
...
TL;DR: 利用微波测量分析两种不同沉积技术制备的Nb₃Sn超导涂层的表面阻抗特性
...
Method: 使用介电负载谐振器在高磁场下进行微波测量，获取Nb₃Sn涂层的表面阻抗数据，分析涡旋动力学和钉扎特性

Result: 两种涂层在通量流电阻率大小和钉扎机制上存在显著差异，但表面电阻值相近，表明存在通过参数折衷优化薄膜性能的空间

Conclusion: 不同沉积技术制备的Nb₃Sn涂层表现出不同的超导特性和涡旋动力学行为，尽管表面电阻相近，但通过参数优化可以进一步提升薄膜性能
